...
import { SettingsStateDiagnosticsRenderer } from './settings_diagnostics_renderer';

describe('SettingsStateDiagnosticsRenderer', () => {
  let renderer: SettingsStateDiagnosticsRenderer;
  let mockSettingsDetails: SettingsDetails;

  beforeEach(() => {
    renderer = new SettingsStateDiagnosticsRenderer();
  ...
  describe('render', () => {
    ...
    it('should render without JSON settings when no configuration values exist', () => {
      jest.spyOn(extensionConfigurationService, 'getChangedConfigurationJSON').mockReturnValue({});
